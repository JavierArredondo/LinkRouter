...
- **The binary is both the app and the CLI.** `main.swift` decides before AppKit starts, because once `NSApplication` is running there is no clean way to exit with a status code. Only the verbs in `CommandLineMode.verbs` divert; anything else — including arguments macOS itself passes on launch — falls through to the GUI.
...
- **Safari profiles are not supported, and the reason is not effort.** Safari exposes no mechanism to target one: the binary takes no profile flag (invoking it just launches the app), it declares no profile-carrying URL scheme (`http`, `https`, `file`, `x-safari-https`), and its scripting dictionary has no notion of profiles at all — `add reading list item`, `do JavaScript`, `search the web`, and nothing about windows or profiles. That is the opposite of Chromium, where `--profile-directory` is documented and verified here. Implementing it would mean driving Safari's UI, which needs Accessibility permission and breaks on any redesign. Do not add it without re-checking whether Apple has published an API.
