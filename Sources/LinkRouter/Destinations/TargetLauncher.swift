...
final class TargetLauncher {
    func launch(_ url: URL, in destination: Destination) async -> Error? {
        ...
        if let profileDirectory = destination.chromeProfileDirectory {
            return await launchChromeProfile(url, profileDirectory: profileDirectory, appURL: appURL)
        }
    ...
    /// forces a fresh process whose command line Chrome's singleton forwards to the live instance —
    /// without it the link lands in whichever profile happens to be frontmost.
    private func launchChromeProfile(_ url: URL, profileDirectory: String, appURL: URL) async -> Error? {
